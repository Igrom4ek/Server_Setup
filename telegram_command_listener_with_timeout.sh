...
# Включаем логирование входящих соединений
...
# Настройка rsyslog для перенаправления логов psad
if ! grep -q "psad" /etc/rsyslog.conf; then
  echo ':msg, contains, "psad" /var/log/psad/alert' | sudo tee -a /etc/rsyslog.conf
  echo '& stop' | sudo tee -a /etc/rsyslog.conf
  sudo systemctl restart rsyslog
fi

# Обновление пути логов в psad.conf
...
log "🛡 Настройка sudo для запуска rkhunter без пароля"
