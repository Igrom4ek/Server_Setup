...
cat > /home/$USERNAME/install_user.sh <<'EOF_USER'
#!/bin/bash
set -e
...
# === Функция: secure_install (от root) ===
secure_install() {
...
# === Функция: verify_install (от пользователя) ===
verify_install() {
...
[[ -f /var/log/auth.log ]] && echo " Лог авторизаций есть" || echo " Нет auth.log"
...
docker ps | grep -q "netdata/netdata" || echo " Образ Netdata может отсутствовать"
...
echo "---  PSAD / RKHUNTER ---"
[[ -f /var/log/psad/alert ]] && echo " psad: лог alert найден" || echo " psad лог не найден"
...
# === Запускаем secure_install от root ===
log "Запускаем secure_install..."
secure_install

# === Запускаем verify_install от имени пользователя ===
log "Запускаем verify_install от имени $USERNAME..."
sudo -i -u "$USERNAME" bash -c "$(declare -f verify_install); verify_install"


# === Установка systemd unit для Telegram-бота ===
log "Создаём systemd unit для Telegram-бота..."
...
systemctl daemon-reload
systemctl enable --now telegram_command_listener.service
log "Telegram-бот установлен как systemd-сервис"
